...
parser.add_argument('-d', '--directory', required=True, help='Full path of directory containing bam files. Files can be'
...
def filter_map_report_files(samples):
    ...
    if analyzed_samples:
        print "Found existing alignment report for " + ', '.join(analyzed_samples)
...
def find_bam_files(samples):
    ...
    for sample in samples:
        ...
        elif len(sample_bam) == 0: not_found_samples.append(sample)
        else: matched_samples.append((sample, sample_bam[0]))

    if not_found_samples:
        print "No final bam file found for " + ', '.join(not_found_samples)
...
def collect_metrics(matched_samples):
    ...
    output = os.path.join(diag_dir, 'picard_report_%A-%a.out')
    error = os.path.join(diag_dir, 'picard_report_%A-%a.err')

    analysis_dir = 'picard_reports-' + os.path.splitext(os.path.basename(csv_file))[0]
    arg_list = []
    ...
    bash_file = os.path.join(script_directory, 'array_scripts', 'picard_report.sh')
    subcall_list = ['sbatch', '--array=0-'+str(array_num), '--error='+str(error), '--output='+str(output), bash_file,
                    '-d', script_directory, '-r', ref_seq]

    subcall_list.extend(arg_list)
    returncode = subprocess.call(subcall_list)
    print "sbatch picard_report.sh executed, return code is " + str(returncode)
    return returncode
