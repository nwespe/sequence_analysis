...
try:
    CLUSTAL_BINARY = subprocess.check_output(["which","clustalw2"]).strip()
except:
    try:
        clustal_pwd = os.path.join(CURDIR, "clustalw2")
        CLUSTAL_BINARY = subprocess.check_output(
                ["which",clustal_pwd]).strip()
    except:
        print("clustalw2 must be in environmental path or in working directory")
        quit()
...
class AnalysisPipeline(object):
    ...
    def generate_gene_dict_from_mutation_dict(self):
        ...
        The mutation dictionary is a list of mutations keyed by the
    ...
    def __init__(self, genome_fasta, chr_name_filename, features_filename):
        ...
        for record in SeqIO.parse(genome_fasta, "fasta"):
            if record.id in chrom_id_to_name_lookup:
                ...
                chrom_num = chrom_id # last value is '2-micron'; isn't this undefined if chr_name_filename isn't given?
...
class Chromosome(object):
    ...
    def parse_introns(self):
        ...
        #Label all parent SeqFeatures with introns using a new attribute, intron
...
class Gene(object):
    ...
    function with the add_mutation function (for >1 mutation/gene).
...
#                location_operator=ref_feature.location_operator
            ...
            # print(ref_feature)
            new_exon_list = []
            for exon_start, exon_end in ref_feature.exon_start_stop_pairs:
                if offset_list:
                    for test_pos, num_bases in offset_list:
...
class PositionSummary(object):
    ...
    Instance is created by Gene class instance at first occurence
    of a mutation (for any sample) in a particular location.
    The mutations are contained in mutation_dict and
    ...
    def is_clone_mutated_from_ancestor_position(self, print_debug):
        """Return True if clone is mutated from the ancestor at position."""
        for mutation in self.mutation_dict.values():
            if print_debug:
            ...
            if (mutation.mutation_type == 'forward mutation' or
                    mutation.mutation_type == 'backmutation'): # possibly modify here to better handle back-mutations?
    ...
    def return_position_mutations(self, sample_name, include_segregant_data):
        """Return mutation to be used when generating mutated sequence.

        This is where the filtering occurs to decide if a mismatch
        gets included in the newly generated mutant sequence.
    ...
    def generate_sample_info_list(self, tabbed_output, mutation_type):
        ...
        sample_info_list = []
        mut_template = \
            '%(smp)s : %(mut)s %(snp)s from %(anc)s to %(chi)s at %(freq)d percent of %(tot)d reads.' # this should be changed for backmutations...
        seg_template = \
        ...
        for (sample_name, mutation) in self.mutation_dict.iteritems():
            ...
            tabbed_output.write( # add number of variant reads
                "%s\t%s\t%s\t%i\t%s\t%s\t%s\t%s\t%i\t%i\t%s\n" % (
                    ...
                    mutation_type))
            # create one-line summary for mutation dict and write to separate output - want info on every sample queried
...
class DataOutput(object):
    ...
    def sort_genes_for_output_nonsegregating(self):
        ...
        self.sorted_genes['Clone variant, nonsynonymous'] = [
            gene for gene in self.analysis.gene_list
            if gene.is_nonsynonymous and gene.clone_mutated_from_ancestor]

        self.sorted_genes['Clone variant, synonymous'] = [
            gene for gene in self.analysis.gene_list
            if gene.type == 'ORF' and not gene.is_nonsynonymous
        ...
        self.sorted_genes['Reference variant, nonsynonymous'] = [
            gene for gene in self.analysis.gene_list
            if gene.is_nonsynonymous
            and not gene.clone_mutated_from_ancestor]

        self.sorted_genes['Reference variant, synonymous'] = [
            gene for gene in self.analysis.gene_list
            if gene.type == 'ORF' and not gene.is_nonsynonymous
            and not gene.clone_mutated_from_ancestor]
    ...
    def write_multiple_clone_output_html(self, include_reference_differences):
        ...
        if include_reference_differences:
            ...
            for gene in self.sorted_genes['Reference variant, nonsynonymous']:
                body_string += self.write_gene(
                    gene, True, self.templates['gene'],
                    self.templates['mutation'],
                    'Reference variant, nonsynonymous')
            body_string += (
                '<h2 id="reference_variant_synonymous">Synonymous, reference variant only:</h2>\n')
            for gene in self.sorted_genes['Reference variant, synonymous']:
                body_string += self.write_gene(
                    gene, False, self.templates['gene'],
                    self.templates['mutation'],
                    'Reference variant, synonymous')

            body_string += (
                '<h2 id="reference_variant_promoter">Promoter, reference variant only:</h2>\n')
            for gene in self.sorted_genes['Reference variant, promoter']:
                body_string += self.write_gene(
                    gene, False, self.templates['gene'],
                    self.templates['mutation'],
    ...
    def write_segregation_html(self, format):
        ...
        for gene in self.sorted_genes['nonsynonymous']:
            body_string += self.write_gene(
                gene, True, self.templates['gene'],
                self.templates['mutation'], 'nonsynonymous')

        body_string += (
            '<h2 id="synonymous">Synonymous ORF segregating mutations:</h2>\n')
        for gene in self.sorted_genes['synonymous']:
            body_string += self.write_gene(
                gene, False, self.templates['gene'],
                self.templates['mutation'], 'synonymous')
...
def main(argv=None):
    ...
    if (all_args.pool_bam and all_args.pool_snp and all_args.pool_indel):
        ...
        pass
        # TODO: multiple clone analysis - argparser is set up to take one additional clone but the pipeline can compare an unlimited number 
